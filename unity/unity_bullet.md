...
- Material上のOffset等UV座標を毎回動的に変更すると別マテリアル扱いになってバッチが走らないため、事前にMaterialごとにUV座標を定義し、Material単位で描画コールした。MaterialPropertyBlockを配列で持っておけば同様の実装になるかもしれない。
